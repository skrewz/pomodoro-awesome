...
local function pomodoro_stop()
  ...
  pomodoro_renotify_timer:stop(pomodoro_renotify_timer)
...
pomodoro_renotify_timer:connect_signal("timeout",
function(c)
  if pomodoro_timer.started then
    r = (pomodoro_time - pomodoro_nbsec) % 60
    pomodoro_notify('End in ' .. math.floor((pomodoro_time - pomodoro_nbsec) / 60) .. ' min ' .. r)
  end
end)
